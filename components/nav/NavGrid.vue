    ...
    <template v-if="query.trim()">
      <div>
        ...
        <div v-else class="flex flex-col items-center justify-center py-16 text-gray-500">
          <svg class="w-12 h-12 mb-3 opacity-50" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
          <p>没有找到相关网站</p>
        </div>
      </div>
    </template>
...
import { navData, categories, getCategoriesWithCount, getLeafCategories, getCategoryById } from '~/data/nav-data'
import type { NavItem, Category } from '~/types/nav'

const { query, activeCategory, selectCategory } = useSearch()
